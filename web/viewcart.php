...
if (isTheseParametersAvailable(array('buyer_id'))) {
    ...
    $stmt = $conn->prepare("SELECT qty,product_name,image,price FROM cart inner join product on cart.p_id=product.p_id where b_id=?");
    $stmt->bind_param("s",$buyer_id);
    $stmt->execute();
    //$tot = 0;
    $stmt->bind_result( $qty,$pname,$image,$price,);
    $cart_details = array();
    while ($stmt->fetch()) {
        $temp = array();
        //$temp['pid'] = $pid;
